
<div class='mainNormal'>
    <div class="m-form m-form_float" style="padding-top:0px;">
    </div>

    <div class='pane pane-color2' style='margin-top:0px;'>
        <table>
            <tbody>
                <tr>
                    <td class='title' colspan='2'>佟掌柜开源作品-后台前段框架-核心开发</td>
                </tr>
                <tr>
                    <td>声明 本着开源精神 可免费用于 企业、个人 使用 ；禁止 二次 加工发布 ！！！</td>
                </tr>                 
                <tr>
                    <td>开发时间 2016-9-1至2017-2-1 工作之外时间 ps 做开源的 真是奉献与牺牲啊</td>
                </tr>    
                <tr>
                    <td>交流群：佟掌柜开源作品交流群 198814859  加群格式 姓名-职业-经验-工作地点</td>
                </tr>                                  
                <tr>
                    <td><a href='http://tongzhangui.com/'>博客地址 http://tongzhangui.com/</a></td>
                </tr>     
                <tr>
                    <td>如果你用着还行，那就 赞助我吧 让我走的更远！<br/><img width='200' src='yidan/images/weixin.jpg' /></td>
                </tr>                   
            </tbody>
        </table>
    </div>

    <div class='pane pane-color1'>
        <table>
            <tbody>
                <tr class='title'>
                    <td  colspan='1'>消息通知</td>
                    <td>添加时间</td>
                </tr>
                                                        <tr>
                        <td><a class='a_alert' data='60'  title='title' href='/sysNotice-detail.html'><i style='margin-right:10px;' class="fa fa-send-o"></i>title</a></td>
                        <td>2016-11-25</td>
                    </tr>
                                                    </tbody>
        </table>
    </div>

</div>

